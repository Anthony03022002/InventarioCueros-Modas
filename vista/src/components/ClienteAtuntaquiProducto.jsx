...
import { getAllPagosAtuntaqui, deletePagosAtuntaqui, updatePagosAtuntaqui } from "../api/generarPagoAtuntaqui.api"; // Importa las funciones de eliminar y actualizar
import Select from "react-select";

export function ClienteAtuntaquiProducto() {
  ...
  const handleSaveEditPago = async () => {
    if (editingPago) {
      ...
      await updatePagosAtuntaqui(editingPago.id, editingPago);
      setEditingPago(null);
      const res = await getAllPagosAtuntaqui(); // Recarga los pagos después de la edición
      const pagosFiltrados = res.data.filter(
        (pago) => pago.cliente === selectedCliente.value
      );
      setPagosCliente(pagosFiltrados);
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center titulos">Resumen de Clientes Atuntaqui</h2>
