...
import { getAllPagosBolivar, deletePagosBolivar, updatePagosBolivar } from "../api/generarPagoBolivar.api"; // Importa las funciones de eliminar y actualizar
import Select from "react-select";

export function ClienteBolivarProducto() {
  ...
  const handleSaveEditPago = async () => {
    if (editingPago) {
      ...
      await updatePagosBolivar(editingPago.id, editingPago);
      setEditingPago(null);
      const res = await getAllPagosBolivar(); // Recarga los pagos después de la edición
      const pagosFiltrados = res.data.filter(
        (pago) => pago.cliente === selectedCliente.value
      );
      setPagosCliente(pagosFiltrados);
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center titulos">Resumen de Clientes Bolivar</h2>
