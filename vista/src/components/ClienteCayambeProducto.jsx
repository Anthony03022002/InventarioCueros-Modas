...
import { getAllPagosCayambe, deletePagosCayambe, updatePagosCayambe } from "../api/generarPagoCayambe.api"; // Importa las funciones de eliminar y actualizar
import Select from "react-select";

export function ClienteCayambeProducto() {
  ...
  const handleSaveEditPago = async () => {
    if (editingPago) {
      ...
      await updatePagosCayambe(editingPago.id, editingPago);
      setEditingPago(null);
      const res = await getAllPagosCayambe(); // Recarga los pagos después de la edición
      const pagosFiltrados = res.data.filter(
        (pago) => pago.cliente === selectedCliente.value
      );
      setPagosCliente(pagosFiltrados);
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center titulos">Resumen de Clientes Cayambe</h2>
