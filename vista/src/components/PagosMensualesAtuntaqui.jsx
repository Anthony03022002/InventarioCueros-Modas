...
import { getVentaAtuntaqui } from "../api/ventasAtuntaqui.api";
import { getAllInventario } from "../api/inventario.api";
import { getAllPagosAtuntaqui } from "../api/generarPagoAtuntaqui.api";

export function PagosMensualesAtuntaqui() {
  const [productoAtuntaqui, setProductoAtuntaqui] = useState([]);
  ...
  useEffect(() => {
    async function cargarProducto() {
      if (id) {
        const { data } = await getVentaAtuntaqui(id);
        setProductoAtuntaqui(Array.isArray(data) ? data : [data]);
      }
    }
    cargarProducto();
  }, [id]);
  ...
  useEffect(() => {
    async function cargarPagosCliente() {
      const res = await getAllPagosAtuntaqui();
  ...
  const findClientNameById = (clientId) => {
    const cliente = clientesAtuntaqui.find((c) => c.id === clientId);
  ...
  return (
    <div className="container mt-3">
      <Link to='/ventasAtuntaqui' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
      <h1 className="titulos">Pagos Mensuales del cliente</h1>
      {productoAtuntaqui.map((producto) => {
          ...
          <div key={producto.id} className="producto-container mb-3">
            <div className="row">
              <div className="col">
                <div className="producto-info">
                  ...
                  className="btn btn-primary"
                   onClick={()=>{
                    navigate(`/ventasAtuntaqui/${producto.id}`)
                  }}
              ...
              <div className="col-auto">
                <button
                  onClick={() => navigate(`/ventasAtuntaqui/${id}/generarPagoAtuntaqui`, { state: { debe } })}
                  className="btn btn-primary"
                >
