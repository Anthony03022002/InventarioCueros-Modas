...
export function ProductoClienteAngelList() {
  ...
  const sortedProductosAngel = productosAngel.slice().sort((a, b) => {
  ...
  const filteredProductosAngel = sortedProductosAngel.filter((productos) => {
    const cliente = clientesAngel.find((cliente) => cliente.id === productos.cliente);
    return cliente?.nombre_completo.toLowerCase().includes(searchTerm.toLowerCase());
  });

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredProductosAngel.slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(filteredProductosAngel.length / itemsPerPage);

  return (
    <div className="container pt-4">
        <h1 className="titulos">Ventas Angel</h1>
