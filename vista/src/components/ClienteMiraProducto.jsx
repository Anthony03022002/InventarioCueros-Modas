...
import { getAllPagosMira, deletePagosMira, updatePagosMira } from "../api/generarPagoMira.api"; // Importa las funciones de eliminar y actualizar
import Select from "react-select";

export function ClienteMiraProducto() {
  ...
  const handleSaveEditPago = async () => {
    if (editingPago) {
      ...
      await updatePagosMira(editingPago.id, editingPago);
      setEditingPago(null);
      const res = await getAllPagosMira(); // Recarga los pagos después de la edición
      const pagosFiltrados = res.data.filter(
        (pago) => pago.cliente === selectedCliente.value
      );
      setPagosCliente(pagosFiltrados);
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center titulos">Resumen de Clientes Mira</h2>
