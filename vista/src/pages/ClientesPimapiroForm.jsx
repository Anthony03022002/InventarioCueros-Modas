...
import {
  createClientesPimampiro,
  deleteClientesPimampiro,
  getClientePimampiro,
  updateClientesPimampiro,
} from "../api/clientesPimampiro.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesPimampiroForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesPimampiro(params.id, data);
      navigate("/clientesPimampiro");
    } else {
      setFormData(data);
  ...
  const handleDelete = async () => {
    await deleteClientesPimampiro(params.id);
    navigate("/clientesPimampiro");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClientePimampiro(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
      <Link to='/clientesPimampiro' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
        <h1 className="titulos">Formulario Clientes Pimampiro</h1>
