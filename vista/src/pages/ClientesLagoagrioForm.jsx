...
import {
  createClientesLagoagrio,
  deleteClientesLagoagrio,
  getClienteLagoagrio,
  updateClientesLagoagrio,
} from "../api/clientesLagoagrio.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesLagoagrioForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesLagoagrio(params.id, data);
      navigate("/clientesLagoagrio");
    } else {
      setFormData(data);
  ...
  const handleDelete = async () => {
    await deleteClientesLagoagrio(params.id);
    navigate("/clientesLagoagrio");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteLagoagrio(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
      <Link to='/clientesLagoagrio' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
        <h1 className="titulos">Formulario Clientes Lagoagrio</h1>
