...
import {
  createClientesOtavalo,
  deleteClientesOtavalo,
  getClienteOtavalo,
  updateClientesOtavalo,
} from "../api/clientesOtavalo.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesOtavaloForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesOtavalo(params.id, data);
      navigate("/clientesOtavalo");
    } else {
      setFormData(data);
  ...
  const handleDelete = async () => {
    await deleteClientesOtavalo(params.id);
    navigate("/clientesOtavalo");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteOtavalo(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
      <Link to='/clientesOtavalo' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
        <h1 className="titulos">Formulario Clientes Otavalo</h1>
