...
import { createPagosLagoagrio } from "../api/generarPagoLagoagrio.api";
import { useNavigate, useParams, useLocation } from "react-router-dom";
import { useEffect} from "react";
import { jsPDF } from "jspdf";

export function GenerarPagoLagoagrioForm() {
  ...
  const recalculateDebe = async (ventaId) => {
    const { data: producto } = await getProductoLagoagrio(ventaId);
    const { data: pagos } = await getAllPagosLagoagrio();
  ...
  const onSubmit = handleSubmit(async (data) => {
    await createPagosLagoagrio(data);
    generatePDF(data);
    navigate(`/ventasLagoagrio/${params.id}/pagosMensualesLagoagrio`);
  });
  ...
  return (
    <div className="container mt-3">
      ...
      >
        <h1 className="titulos">Formulario de pagos Lagoagrio</h1>
