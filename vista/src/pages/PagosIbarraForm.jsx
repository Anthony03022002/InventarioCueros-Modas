...
import {
  ...
  deletePagosIbarra,
  getPagoIbarra,
} from "../api/generarPagoIbarra.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosIbarraForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosIbarra(params.id, data);
    } else {
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoIbarra(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosIbarra(params.id);
    navigate("/ventasIbarra");
  };
