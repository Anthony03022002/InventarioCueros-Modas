...
import {
  createPagosLagoagrio,
  updatePagosLagoagrio,
  deletePagosLagoagrio,
  getPagoLagoagrio,
} from "../api/generarPagoLagoagrio.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosLagoagrioForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosLagoagrio(params.id, data);
    } else {
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoLagoagrio(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosLagoagrio(params.id);
