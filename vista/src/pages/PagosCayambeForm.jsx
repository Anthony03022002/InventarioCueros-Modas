...
import {
  ...
  deletePagosCayambe,
  getPagoCayambe,
} from "../api/generarPagoCayambe.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosCayambeForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosCayambe(params.id, data);
    } else {
      await createPagosCayambe(data);
    }
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoCayambe(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosCayambe(params.id);
    navigate("/ventasCayambe");
  };
  ...
  return (
    <div className="container mt-3">
      ...
      >
        <h1 className="titulos">Formulario de pagos Cayambe</h1>
