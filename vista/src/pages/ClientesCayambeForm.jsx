...
import {
  createClientesCayambe,
  deleteClientesCayambe,
  getClienteCayambe,
  updateClientesCayambe,
} from "../api/clientesCayambe.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesCayambeForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesCayambe(params.id, data);
      navigate("/clientesCayambe");
    } else {
      setFormData(data);
  ...
  const handleDelete = async () => {
    await deleteClientesCayambe(params.id);
    navigate("/clientesCayambe");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteCayambe(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
      <Link to='/clientesCayambe' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
        <h1 className="titulos">Formulario Clientes Cayambe</h1>
