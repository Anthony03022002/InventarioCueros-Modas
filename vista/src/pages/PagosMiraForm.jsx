...
import {
  ...
  deletePagosMira,
  getPagoMira,
} from "../api/generarPagoMira.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosMiraForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosMira(params.id, data);
    } else {
      await createPagosMira(data);
    }
    navigate("/ventasMira");
  });
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoMira(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosMira(params.id);
    navigate("/ventasMira");
  };
  ...
  return (
    <div className="container mt-3">
      ...
      >
        <h1 className="titulos">Formulario de pagos Mira</h1>
