...
import { createPagosAtuntaqui } from "../api/generarPagoAtuntaqui.api";
import { useNavigate, useParams, useLocation } from "react-router-dom";
import { useEffect} from "react";
import { jsPDF } from "jspdf";

export function GenerarPagoAtuntaquiForm() {
  ...
  const recalculateDebe = async (ventaId) => {
    const { data: producto } = await getProductoAtuntaqui(ventaId);
    const { data: pagos } = await getAllPagosAtuntaqui();
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    generatePDF(data);
    navigate(`/ventasAtuntaqui/${params.id}/pagosMensualesAtuntaqui`);
  });
