...
import {
  createClientesBolivar,
  deleteClientesBolivar,
  getClienteBolivar,
  updateClientesBolivar,
} from "../api/clientesBolivar.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesBolivarForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesBolivar(params.id, data);
      navigate("/clientesAtuntaqui");
    } else {
      setFormData(data);
  ...
  const handleCancelSale = async () => {
    setShowSaleModal(false);
    await createClientesBolivar(formData);
    navigate("/clientesBolivar");
  };

  const handleDelete = async () => {
    await deleteClientesBolivar(params.id);
    navigate("/clientesBolivar");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteBolivar(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      <Link to='/clientesBolivar' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
      <div className="flex-grow-1 d-flex justify-content-center">
        <h1 className="titulos">Formulario Clientes Bolivar</h1>
      </div>
