...
import { createPagosIbarra } from "../api/generarPagoIbarra.api";
import { useNavigate, useParams, useLocation } from "react-router-dom";
import { useEffect} from "react";
import { jsPDF } from "jspdf";

export function GenerarPagoIbarraForm() {
  ...
  const recalculateDebe = async (ventaId) => {
    const { data: producto } = await getProductoIbarra(ventaId);
    const { data: pagos } = await getAllPagosIbarra();
  ...
  const onSubmit = handleSubmit(async (data) => {
    await createPagosIbarra(data);
    generatePDF(data);
    navigate(`/ventasIbarra/${params.id}/pagosMensualesIbarra`);
  });
  ...
  return (
    <div className="container mt-3">
      ...
      >
        <h1 className="titulos">Formulario de pagos Ibarra</h1>
