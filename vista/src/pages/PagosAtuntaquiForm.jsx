...
import {
  createPagosAtuntaqui,
  updatePagosAtuntaqui,
  deletePagosAtuntaqui,
  getPagoAtuntaqui,
} from "../api/generarPagoAtuntaqui.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosAtuntaquiForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosAtuntaqui(params.id, data);
    } else {
      await createPagosAtuntaqui(data);
    }
    navigate("/pagosAtuntaqui");
  });

  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoAtuntaqui(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosAtuntaqui(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
        <h1 className="titulos">Formulario de pagos Atuntaqui</h1>
