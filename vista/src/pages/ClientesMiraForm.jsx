...
import {
  ...
  getClienteMira,
  updateClientesMira,
} from "../api/clientesMira.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesMiraForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesMira(params.id, data);
      navigate("/clientesMira");
    } else {
      setFormData(data);
  ...
  const handleDelete = async () => {
    await deleteClientesMira(params.id);
    navigate("/clientesMira");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteMira(params.id);
