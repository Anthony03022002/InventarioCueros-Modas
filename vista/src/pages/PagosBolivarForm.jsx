...
import {
  ...
  deletePagosBolivar,
  getPagoBolivar,
} from "../api/generarPagoBolivar.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosBolivarForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosBolivar(params.id, data);
    } else {
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoBolivar(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosBolivar(params.id);
    navigate("/ventasBolivar");
  };
