...
import {
  creatVentasLagoagrio,
  deleteVentasLagoagrio,
  updateVentasLagoagrio,
  getVentaLagoagrio,
} from "../api/ventasLagoagrio.api";
import { useNavigate, useParams, Link } from "react-router-dom";
import { createVentasHistorial } from "../api/ventasHistorial.api";

export function VentasLagoagrioForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateVentasLagoagrio(params.id, data);
    } else {
      await creatVentasLagoagrio(data);
  ...
  const handleDelete = async () => {
    await deleteVentasLagoagrio(params.id);
    navigate("/ventasLagoagrio");
  };
  ...
  useEffect(() => {
    async function actualizarProducto() {
      if (params.id) {
        const { data } = await getVentaLagoagrio(params.id);
        ...
        const clienteSeleccionado = clientesLagoagrio.find(
    ...
    actualizarProducto();
  }, [params.id, clientesLagoagrio, inventarios]);
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div className="col-md-3">
          ...
          <Select
            ...
            options={clientesLagoagrio.map((cliente) => ({
              value: cliente.id,
