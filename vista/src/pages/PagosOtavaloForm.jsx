...
import {
  ...
  deletePagosOtavalo,
  getPagoOtavalo,
} from "../api/generarPagoOtavalo.api";
import { useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";

export function PagosOtavaloForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updatePagosOtavalo(params.id, data);
    } else {
  ...
  useEffect(() => {
    async function cargarPagos() {
      if (params.id) {
        const { data } = await getPagoOtavalo(params.id);
  ...
  const handleDelete = async () => {
    await deletePagosOtavalo(params.id);
    navigate("/ventasOtavalo");
  };
