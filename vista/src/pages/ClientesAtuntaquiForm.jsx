...
import {
  createClientesAtuntaqui,
  deleteClientesAtuntaqui,
  getClienteAtuntaqui,
  updateClientesAtuntaqui,
} from "../api/clientesAtuntaqui.api";
import { useEffect, useState } from "react";
import "../index.css";

export function ClientesAtuntaquiForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (params.id) {
      await updateClientesAtuntaqui(params.id, data);
  ...
  const handleDelete = async () => {
    await deleteClientesAtuntaqui(params.id);
    navigate("/clientesAtuntaqui");
  };
  ...
  useEffect(() => {
    async function actualizarClientes() {
      if (params.id) {
        const { data } = await getClienteAtuntaqui(params.id);
  ...
  return (
    <div className="container mt-3">
      ...
      >
      <Link to='/clientesAtuntaqui' className="fs-3"><i className="bi bi-arrow-left-circle-fill"></i></Link>
        <h1 className="titulos">Formulario Clientes Atuntaqui</h1>
